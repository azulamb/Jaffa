...
module Jaffa
{
	...
	export class DrawCanvas //implements JaffaDraw.Draw
	{
		...
		drawImageScalingC( imgnum: number, rx: number, ry: number, w: number, h: number, dx: number, dy: number, scale: number, dh: any = undefined )
		{
			if ( dh === undefined )
			{
				this.context.drawImage( this.imgs[imgnum], rx, ry, w, h, dx - w * scale / 2, dy - h * scale / 2, w * scale, h * scale );
			} else
			{
				this.context.drawImage( this.imgs[imgnum], rx, ry, w, h, dx - w * scale / 2, dy - h * scale / 2, scale, dh );
			}
			return true;
		}
